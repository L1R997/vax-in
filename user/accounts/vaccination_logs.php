...
<body id="page-top">
    <div id="wrapper">
        ...
        <div id="content-wrapper" class="d-flex flex-column bg-light">
            <div id="content">
                ...
                // Check if we are viewing a specific animal's log
                $specific_animal_id = isset($_GET['animal_id']) ? $_GET['animal_id'] : null;

                // --- PHP PROCESSING LOGIC ---
                
                // 1. ADD VACCINATION LOG (With Status handling)
                if (isset($_POST['add_log'])) {
                    $animal_id = $_POST['animal_id'];
                    $status = $_POST['status']; // 'Vaccinated' or 'Not Vaccinated'
                
                    // Kung Not Vaccinated, NULL ang vaccine_id. Kung Vaccinated, kunin yung pinili.
                    ...
                    $remarks = trim($_POST['remarks']); // Ito yung magiging Rason kung Not Vaccinated
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
                  ...
                  $stmt->bind_param("isissssss", $animal_id, $status, $vaccine_id, $vaccination_date, $next_due_date, $vaccinator_id, $official_name, $official_designation, $remarks);
                        if ($stmt->execute()) {
                            // LOGGING
                            $log_action = ($status === 'Vaccinated') ? "Recorded a successful vaccination for Animal ID: $animal_id" : "Recorded an unsuccessful vaccination attempt (Reason: $remarks) for Animal ID: $animal_id";
    ...
    $sql = "UPDATE vaccination_tbl 
            SET status=?, vaccine_id=?, vaccination_date=?, next_due_date=?, remarks=? 
            WHERE log_id=?";

    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sisssi", $status, $vaccine_id, $vaccination_date, $next_due_date, $remarks, $log_id);

    if ($stmt->execute()) {
        logSystemActivity($conn, $current_user_id, $current_fullname, "Updated vaccination log ID: $log_id");

        echo "<script>
            Swal.fire('Updated!', 'Vaccination log updated.', 'success')
                                ...
                                <div class="row align-items-center">
                                    ...
                                    <div class="col-md-5 pl-4 border-right"
                                        ...
                                        <p class="mb-1 small text-light">
                                            <i class="fas fa-palette mr-1"></i> Color: <?= $animal_data['color'] ?>
                                            &nbsp;|&nbsp; <i class="fas fa-birthday-cake mr-1"></i> Age: <?= $animal_age_str ?>
                                        </p>
                                        <p class="mb-0 mt-2">
                                    ...
                                    <div class="col-md-5 pl-4 position-relative">
                                        <h5 class="font-weight-bold mb-1">
                                            <i class="fas fa-user mr-2"></i><?= $animal_data['owner_name'] ?>
                                        </h5>
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover table-bordered align-middle" id="logsTable" width="100%"
                                    ...
                                    <tbody>
                                        ...
                                        // BINAGO: Gumamit ng LEFT JOIN sa vaccine_tbl para lumabas kahit NULL ang vaccine_id
    ...
    <td class="text-center align-middle">
        ...
        <button class="btn btn-warning btn-sm mb-1"
            data-toggle="modal"
            data-target="#editLogModal<?= $row['log_id'] ?>">
...
<?php endif; ?>
                                                ...
                                                <div class="modal fade" id="editLogModal<?= $row['log_id'] ?>" tabindex="-1">
    <div class="modal-dialog">
        <form method="POST">
            <div class="modal-content">

                <div class="modal-header bg-warning">
                    <h5 class="modal-title">Edit Vaccination Log</h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <select name="status" class="form-control" required>
                            ...
                            <option value="Not Vaccinated" <?= ($row['status'] == 'Not Vaccinated') ? 'selected' : '' ?>>Not Vaccinated</option>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="vaccine_id" class="form-control">
                            ...
                            <?php
                            $v_query2 = mysqli_query($conn, "SELECT * FROM vaccine_tbl");
                            while ($v2 = mysqli_fetch_assoc($v_query2)) {
                                $selected = ($row['vaccine_id'] == $v2['vaccine_id']) ? 'selected' : '';
                                echo "<option value='{$v2['vaccine_id']}' $selected>{$v2['vaccine_name']}</option>";
                            }
                            ?>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="vaccination_date" class="form-control"
                            value="<?= $row['vaccination_date'] ?>" required>
                    </div>
                ...
                <div class="modal-footer">
                    <button type="submit" name="update_log" class="btn btn-success">
                        Save Changes
                    </button>
                </div>
